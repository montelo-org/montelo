...
export const loader: LoaderFunction = async (args) => {
  ...
  return json<AppLayoutLoader>({ environment, project });
};
...
export default function AppLayout() {
  ...
  const { project, environment } = useLoaderData<AppLayoutLoader>();

  const shouldHideOrgSwitcher = pathname === Routes.app.root;

  return (
    <div className="flex flex-col h-screen w-screen">
      <header className="fixed top-0 w-full h-14 z-10 p-4 bg-background shadow">
        <Header project={project} environment={environment} hideOrgSwitcher={shouldHideOrgSwitcher} />
      </header>
